...
export default function CaseStudyForm({ existing, onDone }: Props) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (existing) {
      ...
      store.create(input, user?.email ?? "unknown");
    }
    onDone();
  };
  ...
  return (
    <form onSubmit={submit} className="bg-surface border border-stroke rounded-2xl p-7 space-y-5">
      ...
      <div className="flex items-center gap-3 pt-2">
        <button type="submit" className="relative rounded-xl py-2.5 px-6 font-body font-semibold text-bg overflow-hidden">
          <span className="absolute inset-0 accent-gradient" />
          <span className="relative z-10">{existing ? "Save changes" : "Create"}</span>
        </button>
