                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {isAdmin && c.status !== "published" && (
                      <button
                        onClick={() => store.setStatus(c.id, "published")}
                        className="font-mono text-xs text-emerald-400 border border-emerald-400/30 hover:bg-emerald-400/10 rounded-full px-3 py-1.5 transition-colors"
                      >
                        Publish
                      </button>
                    )}
                    {isAdmin && c.status === "published" && (
                      <button
                        onClick={() => store.setStatus(c.id, "draft")}
                        className="font-mono text-xs text-muted hover:text-amber-400 border border-stroke hover:border-amber-400/30 rounded-full px-3 py-1.5 transition-colors"
                      >
                        Unpublish
                      </button>
                    )}
                    {isAdmin && (
                      <button
                        onClick={() => {
                          if (confirm(`Delete "${c.title}"? This cannot be undone.`)) store.remove(c.id);
                        }}
                        className="font-mono text-xs text-muted hover:text-rose-400 border border-stroke hover:border-rose-400/30 rounded-full px-3 py-1.5 transition-colors"
                      >
                        Delete
                      </button>
                    )}
